...
class NationalStandardizedMortalityRatio:
    """
    ...
    def process_nvss_data(self, path: str, return_processed_data: bool = False) -> Optional[pd.DataFrame]:
        """
        Process NVSS mortality data from a CSV file to compute mortality counts for all states.
        """
    ...
    def calculate_smr(self) -> Optional[pd.DataFrame]:
        """
        Calculate the SMR by county for the entire USA.
        """
        if self.population_data is None:
